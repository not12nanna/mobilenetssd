...
lineaccesstoken = '+hnibw6e3gZeNe7mNF29cwekFXZ7I0yFvPwEvXCqIjZ3YbQIt/UcLqOSsXgmzmdFoc6MbtMfvoUKPyqk6VBTEAsAkKPWPqB7z0/DQgSNTuMc4RMzxJ+7ORoUSHIbipJ+U9noPRcZLt5wO22ymo0SmwdB04t89/1O/w1cDnyilFU='
...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
            replyObj = TextSendMessage(text="Morning")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "กินข้าวไหม") :
            replyObj = TextSendMessage(text="มาม่าดีกว่า")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "ง่วงยัง") :
            replyObj = TextSendMessage(text="ง่วงมากเวอร์")
            line_bot_api.reply_message(rtoken, replyObj)
        else :                               
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'bots.dialogflow.com'})
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/a3525c8c-adf5-4842-bdd7-276e25e7d8f3"
